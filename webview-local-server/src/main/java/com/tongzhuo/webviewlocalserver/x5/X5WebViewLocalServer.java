package com.tongzhuo.webviewlocalserver.x5;

import com.tencent.smtt.export.external.interfaces.WebResourceRequest;
import com.tencent.smtt.export.external.interfaces.WebResourceResponse;
import com.tongzhuo.webviewlocalserver.AndroidProtocolHandler;
import com.tongzhuo.webviewlocalserver.UriMatcher;
...
public class X5WebViewLocalServer {
    ...
    private static String TAG = "X5WebViewAssetServer";
    ...
    public static class AssetHostingDetails {
        ...
        AssetHostingDetails(Uri httpPrefix, Uri httpsPrefix) {
    ...
    X5WebViewLocalServer(AndroidProtocolHandler protocolHandler, String authority) {
     ...
     */
    public X5WebViewLocalServer(Context context, String authority) {
     ...
     * {@link com.tencent.smtt.sdk.WebViewClient#shouldInterceptRequest(com.tencent.smtt.sdk.WebView,
     * WebResourceRequest)}.
     *
     ...
     * This method should be invoked from within
     * {@link com.tencent.smtt.sdk.WebViewClient#shouldInterceptRequest(com.tencent.smtt.sdk.WebView, String)}.
     *
     ...
     *
     * @param uri     the uri to use the handler for. The scheme and authority (domain) will be
     *                matched
     *                exactly. The path may contain a '*' element which will match a single element
     *                of
     *                a path (so a handler registered for /a/* will be invoked for /a/b and
     *                /a/c.html
     ...
     * @param assetPath        the local path in the application's asset folder which will be made
     *                         available by the server (for example "/www").
     * @param virtualAssetPath the path on the local server under which the assets should be
     *                         hosted.
    ...
    public X5WebViewLocalServer.AssetHostingDetails hostStorage(final String storagePath,
            final String virtualStoragePath) {
        return hostStorage(authority, storagePath, virtualStoragePath, true, true);
    }

    public X5WebViewLocalServer.AssetHostingDetails hostStorage(final String storagePath,
            final String virtualStoragePath, boolean enableHttp, boolean enableHttps) {
        return hostStorage(authority, storagePath, virtualStoragePath, enableHttp, enableHttps);
    }

    public X5WebViewLocalServer.AssetHostingDetails hostStorage(final String domain,
            final String storagePath, final String virtualStoragePath,
            boolean enableHttp, boolean enableHttps) {
        Uri.Builder uriBuilder = new Uri.Builder();
        uriBuilder.scheme(httpScheme);
        uriBuilder.authority(domain);
        uriBuilder.path(virtualStoragePath);

        if (storagePath.indexOf('*') != -1) {
            throw new IllegalArgumentException("storagePath cannot contain the '*' character.");
        }
        if (virtualStoragePath.indexOf('*') != -1) {
            throw new IllegalArgumentException(
                    "virtualStoragePath cannot contain the '*' character.");
        }

        Uri httpPrefix = null;
        Uri httpsPrefix = null;
        X5WebViewLocalServer.PathHandler handler = new X5WebViewLocalServer.PathHandler() {
            @Override
            public InputStream handle(Uri url) {
                InputStream stream = null;
                String path = url.getPath().replaceFirst(virtualStoragePath, storagePath);
                try {
                    stream = protocolHandler.openStorage(path);
                } catch (Exception e) {
                    Log.e(TAG, "Unable to open Storage ：" + e);
                }
                return stream;
            }
        };
        ...
        return new X5WebViewLocalServer.AssetHostingDetails(httpPrefix, httpsPrefix);
    }
     ...
     * @param domain               custom domain on which the assets should be hosted (for example
     *                             "example.com").
     *                             If untrusted content is to be loaded into the WebView it is
     *                             advised to make
     *                             this random.
     * @param virtualResourcesPath the path on the local server under which the resources
     *                             should be hosted.
